...
# st.set_page_config REMOVED — handled by Home.py
...
if not has_geo_data:
    ...
    """)
    # NOTE: st.stop() replaced with early exit via flag — do NOT add st.stop() here
    # it would kill rendering of tabs 4 and 5 in Home.py
else:
    if not TENSORFLOW_AVAILABLE:
        ...
        def load_lstm_model():
            try:
                # Resolve paths relative to project root (one level up from pages/)
                base = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
                model_path   = os.path.join(base, 'energy_predictor.h5')
        ...
        if error:
            ...
            st.info("Make sure energy_predictor.h5, scaler_X.pkl and scaler_y.pkl are in the project root folder.")
            # NOTE: st.stop() removed — do NOT add it back
        else:
            st.success("✅ LSTM model loaded successfully!")

            # ── Prediction Settings (moved from sidebar — no sidebar in this app) ──
            st.markdown("---")
            st.markdown("### ⚙️ Prediction Settings")
            cfg1, cfg2, cfg3 = st.columns(3)
            with cfg1:
                forecast_months = st.slider("Forecast Period (months)", 3, 24, 12, key="lstm_forecast_months")
            with cfg2:
                start_month = st.selectbox("Starting Month", 
                    ['January', 'February', 'March', 'April', 'May', 'June',
                     'July', 'August', 'September', 'October', 'November', 'December'],
                    index=datetime.now().month - 1, key="lstm_start_month")
            with cfg3:
                st.markdown("**Climate Scenario**")
                climate_scenario = st.radio(
                    "Select scenario:",
                    ["Normal", "Wetter (More Monsoon)", "Drier (Less Rain)", "Hotter"],
                    key="lstm_climate_scenario"
                )
            ...
            with tab1:
                ...
                if st.button("Generate LSTM Forecast", type="primary", key="lstm_generate_btn"):
                    ...
                    with st.spinner("Running LSTM predictions..."):
                        ...
                        for i in range(forecast_months):
                            ...
                            predictions_mwh.append(float(scaled_prediction))

                            # Confidence intervals (±15%)
                            confidence_lower.append(float(scaled_prediction) * 0.85)
                            confidence_upper.append(float(scaled_prediction) * 1.15)
                        ...
                        # Store predictions — all values cast to plain Python floats to avoid
                        # JSON serialization errors with NumPy float32/float64 types
                        st.session_state.predictions = {
                            'months': months,
                            'temperatures': [float(t) for t in temperatures],
                            'rainfalls': [float(r) for r in rainfalls],
                            'waterfall_mw': [float(w / hours_per_month) for w in waterfall_predictions],
                            'geo_mw': [float(g / hours_per_month) for g in geo_predictions],
                            'total_mw': [float(p) for p in predictions_mw],
                            'waterfall_mwh': [float(w) for w in waterfall_predictions],
                            'geo_mwh': [float(g) for g in geo_predictions],
                            'total_mwh': [float(e) for e in predictions_mwh],
                            'confidence_lower': [float(c) for c in confidence_lower],
                            'confidence_upper': [float(c) for c in confidence_upper],
                            'total_annual_mwh': float(sum(predictions_mwh)),
            ...
            with tab2:
                ...
                if st.checkbox("Show Training History Graph"):
                    try:
                        ...
                        base = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
                        img = Image.open(os.path.join(base, 'training_history.png'))
                ...
                if 'months' in st.session_state.predictions:
                    ...
                    with col2:
                        summary = {
                            ...
                            'average_monthly_mwh': float(np.mean(st.session_state.predictions['total_mwh'])),
                            'predictions': export_df.to_dict('records')
                        }

                        # Custom JSON encoder to handle any remaining NumPy scalar types
                        # (float32, float64, int32, etc.) that are not natively JSON serializable
                        def numpy_json_encoder(obj):
                            if hasattr(obj, 'item'):
                                return obj.item()  # converts any np scalar to native Python type
                            raise TypeError(f"Object of type {type(obj)} is not JSON serializable")

                        json_str = json.dumps(summary, indent=2, default=numpy_json_encoder)
                        st.download_button(
